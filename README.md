...
# 启动服务（默认使用 SQLite，无需额外配置）
cd backend && source venv/bin/activate
python app/main.py
```

**说明**：
- 默认使用 **SQLite** 数据库（文件：`backend/app/openclaw_enterprise.db`）
- 无需安装 MySQL/PostgreSQL
- 适合快速测试和小规模部署

### 方式二：生产环境部署（PostgreSQL）
```bash
# 1. 安装 PostgreSQL
# Ubuntu/Debian
...
cp .env.example .env
# 修改 DATABASE_URL:
# DATABASE_URL=postgresql://user:password@localhost:5432/openclaw_enterprise
...
- **框架**: FastAPI 0.109+
- **数据库**: 
  - 默认：SQLite 3（无需额外配置，一键启动）
  - 生产：PostgreSQL 14+（需配置 DATABASE_URL）
- **ORM**: SQLAlchemy 2.0
- **认证**: JWT (python-jose)
...
| [数据库指南](docs/database.md) | SQLite/PostgreSQL 配置 |
